...
業務で長時間トランザクションによりロック競合が発生し、後続のトランザクションがタイムアウトまで待たされる問題に遭遇した。

当時、「PostgreSQLはMVCCだから読み取りと書き込みは競合しない」という知識だけで、Writer同士の競合についてはあまり意識していなかった。原因を調査する中で`FOR UPDATE NOWAIT`というオプションを知り、ロック取得を待たずに即座にエラーにすることで応急処置ができた。

この経験から、PostgreSQLのロック競合とNOWAITの使いどころを整理したくなり、実際に競合を起こすコードを書いて動かしてみた。
